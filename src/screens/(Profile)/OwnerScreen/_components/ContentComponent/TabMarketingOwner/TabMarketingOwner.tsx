...
export const TabMarketingOwner = () => {
    ...
    const [advertising, setAdvertising] =
        useState<IDataAdvertisingCookie | null>(null);
    ...
    function deleteChoice(indexItem: number) {
        ...
        filterArray.length > 0
            ? cookie.set("advertisingNotPay", {
                  ...
                  price: advertising
                      ? advertising?.price -
                        advertising?.data[indexItem].price.allPrice
                      : 0,
                  data: filterArray,
              })
        ...
        <>
            ...
            {advertising && (
                <div className={style.choice}>
                    <h2 className={style.choice_title}>
                        {t("marketingTab.subTitle")}
                    </h2>
                        ...
                        <ul className={style.choice_list}>
                            ...
                            {advertising.data?.map((item, index) => {
                                return (
                                    <li
                                        className={
                                            style.choice_content_list_item
                                        }
                                        key={index + 10000}
                                    >
                                        <span>{item.type}</span>
                                        ...
                                        <span>
                                            {item.price.allPrice}р. /{" "}
                                            {item.price.priceOne}р.
                                        </span>
                                        <IconDelete
                                            className={
                                                style.choice_content_list_item_button
                                            }
