...
import { IDataCurrency, IDataWalletHistory } from "@/models/IType";
  ...
  return (
    <>
      ...
      <div className={style.account}>
        ...
        <div className={style.account_content}>
          {currencyData?.map((item) => {
            return (
              <div key={item.id} className={style.account_content_item}>
                <div className={style.account_content_item_count}>
                  {item.count} {item.currency.name}({item.currency.value})
                </div>
                ...
                <Button
                  ...
                  className={style.account_content_item_refillBut}
                />
              </div>
            );
          })}
      ...
      <div className={style.history}>
        <h4 className={style.history_title}>{t("walletTab.historyPayment")}</h4>
        <div className={style.history_content}>
          ...
          <ul className={style.history_content_list}>
            {historyData?.map((item) => {
              return (
                <li key={item.id} className={style.history_content_list_item}>
                  <span>{item.id}</span>
